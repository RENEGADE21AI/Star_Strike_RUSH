function drawPressButton(rect, label, pressed, baseFill, stroke = "rgba(120,255,180,0.62)") {
  ...
  fill.addColorStop(0, pressed ? "rgba(112,255,205,0.27)" : "rgba(28,72,78,0.60)");
  fill.addColorStop(0.52, pressed ? "rgba(62,218,164,0.23)" : baseFill);
  fill.addColorStop(1, pressed ? "rgba(20,100,82,0.34)" : "rgba(2,20,25,0.82)");
  ctx.fillStyle = fill;
  ctx.shadowColor = pressed ? "rgba(112,255,205,0.30)" : "rgba(76,226,255,0.14)";
  ctx.shadowBlur = pressed ? 4 : 9;
  ctx.beginPath();
...
function drawSimpleButton(rect, label, stroke = "rgba(255,255,255,0.28)") {
  ctx.save();
  const radius = Math.min(7, rect.h * 0.20);
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(rect.x + radius, rect.y + 2);
  ctx.lineTo(rect.x + rect.w - radius, rect.y + 2);
  ctx.stroke();
  ctx.fillStyle = "#fff";
  ctx.font = FONT_BUTTON;
  ctx.textAlign = "center";
