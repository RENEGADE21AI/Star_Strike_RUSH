function drawTitleAndButtons() {
  ...
  const dock = getTitleMetaDockRect();
  const dockX = dock.x;
  const dockY = dock.y;
  const dockW = dock.w;
  const dockH = dock.h;
  ...
  ctx.beginPath();
  ctx.moveTo(dockX + 14, dockY + 23);
  ctx.lineTo(dockX + dockW - 14, dockY + 23);
  ctx.stroke();
  ctx.font = "900 9px 'Arial Narrow', Arial, sans-serif";
  ctx.textAlign = "center";
  ctx.textBaseline = "top";
  ctx.fillStyle = "rgba(120,210,255,0.70)";
  ctx.fillText("PILOT COMMAND", dockX + dockW / 2, dockY + 6);
  ctx.restore();
  ...
  const activeAccent = achievementsActive ? "rgba(255,230,128,0.86)" : progressActive ? "rgba(120,255,180,0.86)" : recordsActive ? "rgba(120,210,255,0.86)" : codexActive ? "rgba(120,255,180,0.86)" : "rgba(255,255,255,0.16)";
  ctx.save();
  ctx.strokeStyle = activeAccent;
  ctx.globalAlpha = 0.58 + (achievementsActive || progressActive || recordsActive || codexActive ? Math.sin(state.frame * 0.12) * 0.14 : 0);
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(dockX + 14, dockY + dockH - 10);
  ctx.lineTo(dockX + dockW - 14, dockY + dockH - 10);
  ctx.stroke();
  ctx.restore();
  ctx.save();
