...
test("tutorial advancement requires the real action for every mechanic lesson", () => {
  ...
  assert.equal(api.tutorialObjectiveComplete(director, { ghostUses: 1, crossedGhostLane: false }), false);
  assert.equal(api.tutorialObjectiveComplete(director, { ghostUses: 1, crossedGhostLane: true }), true);
...
test("Colonel Vega dialogue stays concise and uses the current call sign", () => {
  const api = loadDirector();
